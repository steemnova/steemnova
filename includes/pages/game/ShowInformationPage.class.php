...
class ShowInformationPage extends AbstractGamePage
{
        ...
        if (in_array($elementID, $reslist['fleet'])) {
            ...
            foreach ($fleetIDs as $fleetID) {
                if (isset($CombatCaps[$elementID]['sd']) && !empty($CombatCaps[$elementID]['sd'][$fleetID])) {
                    $FleetInfo['rapidfire']['to'][$fleetID] = $this->calculateRapidFire($CombatCaps[$elementID]['sd'][$fleetID]);
                }

                if (isset($CombatCaps[$fleetID]['sd']) && !empty($CombatCaps[$fleetID]['sd'][$elementID])) {
                    $FleetInfo['rapidfire']['from'][$fleetID] = $this->calculateRapidFire($CombatCaps[$fleetID]['sd'][$elementID]);
                }
            }
        }
    ...
    protected function calculateRapidFire($rapidFire)
    {
        return round(100 * ($rapidFire - 1) / $rapidFire);
    }
}
