...
function ShowAccountEditorPage()
{
    ...
    switch ($_GET['edit']) {
        ...
        case 'ships':
            if ($_POST) {
                $before1 = Database::get()->selectSingle("SELECT * FROM %%PLANETS%% WHERE `id` = :planetId", ["planetId" => HTTP::_GP('id', 0)]);
                $before = array();
                $after = array();
        ...
        case 'buildings':
            if ($_POST) {
                $PlanetData = Database::get()->selectSingle("SELECT * FROM %%PLANETS%% WHERE `id` = :planetId", [":planetId" => HTTP::_GP('id', 0)]);
                if (!isset($PlanetData)) {
        ...
        case 'researchs':
            if ($_POST) {
                $before1 = Database::get()->selectSingle("SELECT * FROM %%USERS%% WHERE `id` = :userId", [":userId" => HTTP::_GP('id', 0)]);
                $before = array();
                $after = array();
        ...
        case 'personal':
            if ($_POST) {
                ...
                $before = Database::get()->selectSingle("SELECT `username`,`email`,`email_2`,`password`,`urlaubs_modus`,`urlaubs_until` FROM %%USERS%% WHERE `id` = '" . HTTP::_GP('id', 0) . "';",[]);
                $after = array();
                ...
                Database::get()->update($PersonalQuery,[]);
        ...
        case 'planets':
            if ($_POST) {
                ...
                if (!empty($name))
                    Database::get()->update("UPDATE %%PLANETS%% SET `name` = '" . $GLOBALS['DATABASE']->sql_escape($name) . "' WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';",[]);
                ...
                if ($buildings == 'on') {
                    ...
                    Database::get()->update("UPDATE %%PLANETS%% SET " . implode(', ', $BUILD) . " WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';",[]);
                }
                ...
                if ($ships == 'on') {
                    ...
                    Database::get()->update("UPDATE %%PLANETS%% SET " . implode(', ', $SHIPS) . " WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");
                }
                ...
                if ($defenses == 'on') {
                    ...
                    Database::get()->update("UPDATE %%PLANETS%% SET " . implode(', ', $DEFS) . " WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");
                }

                if ($c_hangar == 'on')
                    Database::get()->update("UPDATE %%PLANETS%% SET `b_hangar` = '0', `b_hangar_plus` = '0', `b_hangar_id` = '' WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");

                if ($c_buildings == 'on')
                    Database::get()->update("UPDATE %%PLANETS%% SET `b_building` = '0', `b_building_id` = '' WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");

                if (!empty($diameter))
                    Database::get()->update("UPDATE %%PLANETS%% SET `diameter` = '" . $diameter . "' WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");

                if (!empty($fields))
                    Database::get()->update("UPDATE %%PLANETS%% SET `field_max` = '" . $fields . "' WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");
                ...
                if ($change_pos == 'on' && $galaxy > 0 && $system > 0 && $planet > 0 && $galaxy <= Config::get(Universe::getEmulated())->max_galaxy && $system <= Config::get(Universe::getEmulated())->max_system && $planet <= Config::get(Universe::getEmulated())->max_planets) {
                    $P = Database::get()->selectSingle("SELECT galaxy,system,planet,planet_type FROM %%PLANETS%% WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");
                    if ($P['planet_type'] == '1') {
                        ...
                        Database::get()->update("UPDATE %%PLANETS%% SET `galaxy` = '" . $galaxy . "', `system` = '" . $system . "', `planet` = '" . $planet . "' WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "';");
                        ...
                        Database::get()->update("UPDATE %%PLANETS%% SET `galaxy` = '" . $galaxy . "', `system` = '" . $system . "', `planet` = '" . $planet . "', `id_owner` = '" . $QMOON2['id_owner'] . "' WHERE `id` = '" . $id . "' AND `universe` = '" . Universe::getEmulated() . "' AND `planet_type` = '3';");
                    }
                }
        ...
        case 'alliances':
            if ($_POST) {
                ...
                $QueryF = Database::get()->selectSingle("SELECT * FROM %%ALLIANCE%% WHERE `id` = :allyId AND `ally_universe` = :uniId;", [":allyId"=>$id, ":uniId" =>Universe::getEmulated()]);

                if (!empty($name))
                    Database::get()->update("UPDATE %%ALLIANCE%% SET `ally_name` = '" . $name . "' WHERE `id` = '" . $id . "' AND `ally_universe` = '" . Universe::getEmulated() . "';");

                if (!empty($tag))
                    Database::get()->update("UPDATE %%ALLIANCE%% SET `ally_tag` = '" . $tag . "' WHERE `id` = '" . $id . "' AND `ally_universe` = '" . Universe::getEmulated() . "';");
                ...
                if (!empty($externo))
                    Database::get()->update("UPDATE %%ALLIANCE%% SET `ally_description` = '" . $externo . "' WHERE `id` = '" . $id . "' AND `ally_universe` = '" . Universe::getEmulated() . "';");

                if (!empty($interno))
                    Database::get()->update("UPDATE %%ALLIANCE%% SET `ally_text` = '" . $interno . "' WHERE `id` = '" . $id . "' AND `ally_universe` = '" . Universe::getEmulated() . "';");

                if (!empty($solicitud))
                    Database::get()->update("UPDATE %%ALLIANCE%% SET `ally_request` = '" . $solicitud . "' WHERE `id` = '" . $id . "' AND `ally_universe` = '" . Universe::getEmulated() . "';");

                if ($delete == 'on') {
                    Database::get()->update("DELETE FROM %%ALLIANCE%% WHERE `id` = '" . $id . "' AND `ally_universe` = '" . Universe::getEmulated() . "';UPDATE %%USERS%% SET `ally_id` = '0', `ally_rank_id` = '0', `ally_register_time` = '0' WHERE `ally_id` = '" . $id . "';");
                }

                if (!empty($delete_u)) {
                    Database::get()->update("UPDATE %%ALLIANCE%% SET `ally_members` = ally_members - 1 WHERE `id` = '" . $id . "' AND `ally_universe` = '" . Universe::getEmulated() . "';UPDATE %%USERS%% SET `ally_id` = '0', `ally_rank_id` = '0', `ally_register_time` = '0' WHERE `id` = '" . $delete_u . "' AND `ally_id` = '" . $id . "';");
                }
