...
{block name="content"}
    <form action="game.php?page=messages" method="post">
        ...
        <table id="messagestable" class="table table-striped">
            {if $MessID != 999}
                <tr>
                    <td colspan="4">
                        ...
                        <input value="{$LNG.mg_confirm}" type="submit" name="submitTop" class="form-control">
                    </td>
                </tr>
            {/if}
            <tr>
                <td class="right" colspan="4">{$LNG.mg_page}:
                    {if $page != 1}<a href="#"´ onclick="Message.getMessages({$MessID}, {$page - 1});return false;">&laquo;</a>&nbsp;{/if}
                    {for $site=1 to $maxPage}<a href="#" onclick="Message.getMessages({$MessID}, {$site});return false;">{if $site == $page}<b>[{$site}]</b>{else}[{$site}]{/if}</a>{if $site != $maxPage}&nbsp;{/if}{/for}{if $page != $maxPage}&nbsp;<a href="#"
                    ...
                    <td>{$Message.subject}
                        {if $Message.type == 1 && $MessID != 999}
                            <a href="#"
                               ...
                               title="{$LNG.mg_answer_to} {strip_tags($Message.from)}"><img src="{$dpath}img/m.gif"
                                                                                            border="0"></a>
                        {/if}
                    </td>
                </tr>
            ...
            {/foreach}
            <tr style="height: 20px;">
                <td class="right" colspan="4">{$LNG.mg_page}: {if $page != 1}<a href="#"
                                                                                onclick="Message.getMessages({$MessID}, 1);return false;">
                            &laquo;</a>&nbsp;{/if}{for $site=1 to $maxPage}<a href="#"
                                                                              onclick="Message.getMessages({$MessID}, {$site});return false;">{if $site == $page}
                            <b>[{$site}]</b>{else}[{$site}]{/if}
                        </a>{if $site != $maxPage}&nbsp;{/if}{/for}{if $page != $maxPage}&nbsp;<a href="#"
