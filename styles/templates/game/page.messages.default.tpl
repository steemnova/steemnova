                ...
                <ul class="list-group">
                    {foreach $CategoryList as $CategoryID => $CategoryRow}
                        <li class="list-group-item"><a
                                    href="game.php?page=messages&category={$CategoryID}">{$LNG.mg_type.{$CategoryID}}</a>
                            <span class="badge bg-primary rounded-pill"
                                  id="unread_{$CategoryID}">{$CategoryRow.unread}</span>
                            <span class="badge bg-primary rounded-pill"
                    ...
                    <table id="messagestable" class="table table-striped">
                            ...
                            <tr id="message_{$Message.id}"
                                ...
                                <td>{$Message.subject}
                                    ...
                                    {if $MessID != 999}<a href="#"
                                                          onclick="Message.delMessage({$Message.id});return false;"><img
                        ...
                        <tr style="height: 20px;">
                            <td class="right" colspan="4">{$LNG.mg_page}: {if $page != 1}<a
                                    ...
                                    href="game.php?page=messages&category={$MessID}&side={$site}">{if $site == $page}<b>
                                        [{$site}
