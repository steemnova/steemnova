...
<div class="card">
        ...
        {foreach from=$TechTreeList key=$elementID item=$requireList name=techniques}
            {if !is_array($requireList)}
                <li class="nav-item" role="presentation">
                    <button class="nav-link p-3 {if $smarty.foreach.techniques.iteration == 1}active{/if}"
                            id="tab-{$requireList}" data-bs-toggle="tab"
    ...
    <div class="tab-content" id="myTabContent">
        ...
        <div class="tab-pane fade {if $smarty.foreach.techniques.iteration == 1}show active{/if}"
             id="content-{$requireList}" role="tabpanel" aria-labelledby="tab-{$requireList}">
